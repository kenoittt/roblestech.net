...
/*
 * GradualBlur — from React Bits (reactbits.dev), JavaScript + CSS variant.
 * Copied as published; the only change is note 1 below.
 *
 *   1. prefers-reduced-motion is respected. Nothing here animates on a timer,
 *      but the reveal transition and the scroll-triggered fade are motion, and
 *      the rest of this site turns those off when asked.
 *
 * The listing names mathjs as a dependency. The source does not import it —
 * the only imports are React and this component's stylesheet — so it is not
 * installed. Adding an unused maths library to a marketing site's bundle to
 * satisfy a listing would be the wrong trade.
 */
...
function GradualBlur(props) {
  ...
  const containerStyle = useMemo(() => {
    ...
    const baseStyle = {
      ...
      transition: config.animated && !still ? `opacity ${config.duration} ${config.easing}` : undefined,
