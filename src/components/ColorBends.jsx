 ...
 *      headline and is pointer-events:none, so it would never receive an event
 *      and the mouse influence and parallax would be dead. Listening on the
 *      window keeps the same container-relative maths and makes them work.
 *      The default is still the container, so an inline use behaves as
 *      published.
 *
 *   2. prefers-reduced-motion renders one frame and stops instead of running
...
export default function ColorBends({
  ...
  useEffect(() => {
    ...
    /* Reduced motion: draw the pattern once and leave it there. */
  ...
  useEffect(() => {
    ...
    /* Container-relative coordinates either way; only the element being
       listened to changes. See note 1 at the top of this file. */
    const source = pointerSource === 'window' ? window : container;
