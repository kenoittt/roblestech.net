...
export const POST: APIRoute = async (context) => {
  ...
  const destination = String(form.get('destination_name') ?? '').trim();
  ...
  return context.redirect(`/trips/${(data as any).id}?ok=` + encodeURIComponent(destination ? `Trip to ${destination} started — add dates, flights and lodging.` : 'Trip created.'));
};
