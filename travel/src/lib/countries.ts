/*
 * Country list (ISO 3166-1 alpha-2) for the searchable passport/location
 * dropdowns and for visa-rule lookups. Names are what users see and store;
 * codeOf() resolves a stored name back to its code.
 */
...
export const codeOf = (name: string | null | undefined): string | null => {
  if (!name) return null;
  const n = name.trim().toLowerCase();
  return COUNTRIES.find((c) => c.name.toLowerCase() === n || c.code.toLowerCase() === n)?.code ?? null;
};
